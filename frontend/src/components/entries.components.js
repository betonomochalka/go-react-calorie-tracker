...
import {Button, Form, Container, Modal, ModalTitle, ModalBody} from 'react-bootstrap';
...
const Entries =() =>{
    ...
    return(
        <div>
            <Container>
        <Button onClick={() => setAddNewEntry}>Track today`s calories</Button>
            </Container>
            <Container>
        {entries != null && entries.map((entry, i) =>(
            ...
            <Modal show={changeIngredient.change} on hide={() => setChangeIngredient({"change": false, "id": 0})} centered>
            <Modal.Header closeButton>
    ...
    function addSingleEntry(){
        ...
        var url = "http://localhost:8000/entries/create"
        axios.post(url, {
            "ingredionts": newEntry.ingredients,
            "dish": newEntry.dish,
